...
<div style="margin-bottom:24px;">
    <h2 style="font-size:20px;font-weight:700;">My Patients</h2>
</div>
    ...
    <table style="width:100%;border-collapse:collapse;">
        <thead>
            <tr style="background:#f8f9fa;border-bottom:2px solid #eee;">
                ...
                <th style="text-align:left;padding:14px 20px;font-size:13px;font-weight:600;color:#666;">MRN</th>
                <th style="text-align:left;padding:14px 20px;font-size:13px;font-weight:600;color:#666;">Last Visit</th>
        ...
        <tbody>
            <?php if (empty($patients)): ?>
                <tr>
                    <td colspan="4" style="padding:40px 20px;text-align:center;color:#999;font-size:14px;">No patients found.</td>
                </tr>
            <?php else: ?>
                <?php foreach ($patients as $patient): ?>
                    <tr style="border-bottom:1px solid #f0f0f0;transition:background 0.15s;" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background='#fff'">
                        <td style="padding:14px 20px;font-size:14px;color:#333;font-weight:500;"><?= htmlspecialchars($patient['name'] ?? '') ?></td>
                        <td style="padding:14px 20px;font-size:14px;color:#666;font-family:monospace;"><?= htmlspecialchars($patient['mrn'] ?? '') ?></td>
                        <td style="padding:14px 20px;font-size:14px;color:#333;"><?= htmlspecialchars($patient['last_visit'] ?? 'N/A') ?></td>
                        <td style="padding:14px 20px;">
                            <a href="/provider/patients/<?= (int)($patient['id'] ?? 0) ?>" style="display:inline-block;padding:6px 14px;background:#4fc3f7;color:#1a1a2e;border-radius:6px;text-decoration:none;font-size:12px;font-weight:600;">View</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php endif; ?>
